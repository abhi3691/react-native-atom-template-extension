...
async function generateApp(fileName:string,description: string, apiKey: string): Promise<void> {
  const config = {
    headers: {
  ...
  try {
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        if (projectRoot) {
          const directoryPath = path.join(projectRoot, 'src');
          const filePath = path.join(directoryPath, `${fileName}.tsx`);
